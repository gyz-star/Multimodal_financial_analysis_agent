...
from planning.toolkit import stock_analysis, fund_analysis,deposit_analyze
...
tool_name_dict = {
    ...
    "deposit_analyze": "机构经营情况分析工具"
}

def analyze_step_with_model_and_tools(step, image_analysis_report):
    try:
        model = "qwen-plus-latest"
        tools = [stock_analysis, fund_analysis,deposit_analyze]
        ...
        if len(result.tool_calls) == 0:
            ...
            st.write(f"**状态**: 调用工具：{', '.join([tool_name_dict.get(tc.get('name')) for tc in result.tool_calls])}")
            tool_messages = []
