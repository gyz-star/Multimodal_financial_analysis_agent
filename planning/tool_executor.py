...
class ToolExecutor:
    ...
    async def deposit_analyze(
        ...
        """
           ...
           Args:
                parameters: 机构名称
                step_content: 步骤描述文本，若parameters中无机构名称，将从文本中提取其他名字
                progress_callback: 进度回调函数，接收两个参数：进度信息（str）和进度值（0-1的float）
           Returns:
        ...
        async with McpClient("http://localhost:3000/sse") as mcp:
            ...
            raw = await mcp.call_tool("get_deposit_by_id", {"deposit_id": parameters.get('机构名称')})
            # sleep(1)
    ...
    # async def main(self):
    #     deposit_id = "思迈特银行"
    #     analysis = await self.deposit_analyze(deposit_id, "", None)  # 复用 _analyze
    #     print("\n【存款分析】\n", analysis)
